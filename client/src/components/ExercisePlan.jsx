    ...
    <div className="space-y-6">
      {plan.specialNotes && (
        <div className="card p-5 bg-amber-500/5 border-amber-500/30">
          ...
          <p className="text-sm text-amber-100/90 mt-2">{plan.specialNotes}</p>
        </div>
      )}

      <div>
        <h3 className="font-semibold mb-3 text-lg">Weekly schedule</h3>
            ...
            return (
              <div key={key} className="card p-5 hover:border-teal/30 transition">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`text-xs px-2.5 py-0.5 rounded-full border ${cls}`}>{day.type || '—'}</span>
                </div>
                <p className="text-sm text-white/80 leading-relaxed">{day.exercises || 'No exercises listed.'}</p>
        ...
        {plan.warmup && (
          <div className="card p-5">
            <h3 className="font-semibold flex items-center gap-2 text-teal">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22a10 10 0 1 0-10-10"/><path d="M12 6v6l4 2"/></svg>
              Warm-up routine
            </h3>
            <p className="text-sm text-white/70 mt-2">{plan.warmup}</p>
          </div>
        )}
        {plan.cooldown && (
          <div className="card p-5">
            ...
            <p className="text-sm text-white/70 mt-2">{plan.cooldown}</p>
          </div>
        )}
      </div>
